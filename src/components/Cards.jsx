...
const Cards = (props) => {
    ...
    const isSaved = savedQuotesArr?.some(item => item.id == props.id) ; 
      ...
      onClick={ () => 
            {
              if (isSaved)
              {
                  setSavedQuotesArray( prev => prev.filter(item => item.id != props.id) )  ;
              }
              else
              {    
                  setSavedQuotesArray( prev => [...prev , {id:props.id , author : props.author , quote:props.quote} ] ) ;
              }
            }
       }
      >
